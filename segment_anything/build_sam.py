...
def _build_sam(
    ...
    if checkpoint is not None and os.path.isfile(checkpoint):
        print("loading from " + str(checkpoint))
        with open(checkpoint, "rb") as f:
            state_dict = torch.load(f, map_location="cpu")
        sam.load_state_dict(state_dict, strict=True)
    elif checkpoint is not None and os.path.isdir(checkpoint):
        print("loading from " + str(checkpoint))
        with open(os.path.join(checkpoint, "image_encoder.pth"), "rb") as f_encoder:
            state_dict_encoder = torch.load(f_encoder, map_location="cpu")
        ...
        with open(os.path.join(checkpoint, "prompt_encoder.pth"), "rb") as f_prompt:
            state_dict_prompt = torch.load(f_prompt, map_location="cpu")
        ...
        with open(os.path.join(checkpoint, "mask_decoder.pth"), "rb") as f_decoder:
            state_dict_decoder = torch.load(f_decoder, map_location="cpu")
